...
const TEACUP_TOP_SVG = `
<svg class="teacup-top" viewBox="0 0 200 190" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
  <ellipse class="teapot-line" cx="100" cy="108" rx="88" ry="30" />
  <path class="teapot-line" d="M176 98 C198 100 208 108 208 116 C208 124 198 130 178 128" />
  <ellipse class="teapot-line cup-outline" cx="100" cy="105" rx="58" ry="52" />
  <ellipse class="tea-surface" cx="100" cy="106" rx="47" ry="42" />
  <ellipse class="tea-shine" cx="84" cy="90" rx="12" ry="7" />
</svg>`;
...
export function renderPourPage(root) {
  ...
  function startPour() {
    ...
    scene.classList.remove('is-filled', 'is-top');

    if (reduceMotion) {
      // Skip the large movement; do a quick fade straight to the result.
      const t = setTimeout(() => {
        scene.classList.add('is-filled', 'is-top');
        showResult(chosen);
      }, 250);
      pourTimers.push(t);
      return;
    ...
    const t5 = setTimeout(() => scene.classList.add('is-top'), 2850);
  ...
  function resetToIdle() {
    ...
    scene.classList.remove('is-pouring', 'is-tilted', 'is-streaming', 'is-filling', 'is-filled', 'is-top');
    resultEl.hidden = true;
    completionEl.hidden = true;
    emptyEl.hidden = true;
  }
